...
$sqlfairy = "SELECT * FROM books WHERE Category = 'fairy'";
$resultinfairy = mysqli_query($conn2,$sqlfairy);
$fields = mysqli_num_fields($resultinfairy);
...
<?php
    $flag = 3;
    while ($rowfairy=mysqli_fetch_array($resultinfairy)){
        if($flag % 3 == 0){ 
            echo "<tr>";
        }?>
    <div>
    <td class="td"><a href="homepage_detail.php?detitle=<?=$rowfairy['Title']?>"><img src="book_cover/<?php echo$rowfairy['imageb']; ?>" height="300px" width="200px"></a><br>
    <id="title">Title:<?php $title = $rowfairy[1]; echo "$rowfairy[1]";?><br>
    Price:<?php echo "$rowfairy[6]"; ?><br>
    <?php if(isset($_SESSION["username"]) && isset($_SESSION["password"]) && $_SESSION["username"] && $_SESSION["password"]){ ?>
