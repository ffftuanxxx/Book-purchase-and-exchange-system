...
$sqlhistory = "SELECT * FROM books WHERE Category = 'history'";
$resultinhistory = mysqli_query($conn2,$sqlhistory);
$fields = mysqli_num_fields($resultinhistory);
...
<?php
    $flag = 3;
    while ($rowhistory=mysqli_fetch_array($resultinhistory)){
        if($flag % 3 == 0){ 
            echo "<tr>";
        }?>
    <div>
    <td class="td"><a href="homepage_detail.php?detitle=<?=$rowhistory['Title']?>"><img src="book_cover/<?php echo$rowhistory['imageb']; ?>" height="300px" width="200px"></a><br>
    <id="title">Title:<?php $title = $rowhistory[1]; echo "$rowhistory[1]";?><br>
    Price:<?php echo "$rowhistory[6]"; ?><br>
    <?php if(isset($_SESSION["username"]) && isset($_SESSION["password"]) && $_SESSION["username"] && $_SESSION["password"]){ ?>
