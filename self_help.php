...
$sqlselfhelp = "SELECT * FROM books WHERE Category = 'self help'";
$resultinselfhelp = mysqli_query($conn2,$sqlselfhelp);
$fields = mysqli_num_fields($resultinselfhelp);
...
<?php
    $flag = 3;
    while ($rowselfhelp=mysqli_fetch_array($resultinselfhelp)){
        if($flag % 3 == 0){ 
            echo "<tr>";
        }?>
    <div>
    <td class="td"><a href="homepage_detail.php?detitle=<?=$rowselfhelp['Title']?>"><img src="book_cover/<?php echo$rowselfhelp['imageb']; ?>" height="300px" width="200px"></a><br>
    <id="title">Title:<?php $title = $rowselfhelp[1]; echo "$rowselfhelp[1]";?><br>
    Price:<?php echo "$rowselfhelp[6]"; ?><br>
    <?php if(isset($_SESSION["username"]) && isset($_SESSION["password"]) && $_SESSION["username"] && $_SESSION["password"]){ ?>
