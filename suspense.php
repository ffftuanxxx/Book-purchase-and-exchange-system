...
$sqlsuspense = "SELECT * FROM books WHERE Category = 'suspense'";
$resultinsuspense = mysqli_query($conn2,$sqlsuspense);
$fields = mysqli_num_fields($resultinsuspense);
...
<?php
    $flag = 3;
    while ($rowsuspense=mysqli_fetch_array($resultinsuspense)){
        if($flag % 3 == 0){ 
            echo "<tr>";
        }?>
    <div>
    <td class="td"><a href="homepage_detail.php?detitle=<?=$rowsuspense['Title']?>"><img src="book_cover/<?php echo$rowsuspense['imageb']; ?>" height="300px" width="200px"></a><br>
    <id="title">Title:<?php $title = $rowsuspense[1]; echo "$rowsuspense[1]";?><br>
    Price:<?php echo "$rowsuspense[6]"; ?><br>
    <?php if(isset($_SESSION["username"]) && isset($_SESSION["password"]) && $_SESSION["username"] && $_SESSION["password"]){ ?>
